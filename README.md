# CDC EPHTracking Rapid Address De-Identifier (RAD) Tool <img src="CDC_Tracking_Combined.jpg" align="left" height=140/>
...
Rapid Address De-Identifier (RAD) is a Java Swing based thick client application that is useful in de-identifying sensitive datasets. The tool takes in a sensitive dataset that has address level data and assigns each unique address a GUID. The resulting output files generated by the tool can then be used to geocode the data or use it as is depending on the use case.
  
### Typical Use Case
![Geocoding Use Case](https://github.com/CDCgov/EPHTracking-RAD/blob/master/RAD.png)
